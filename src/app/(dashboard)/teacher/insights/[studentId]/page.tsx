...
import { ArrowLeft, AlertTriangle, TrendingUp, TrendingDown, Minus, Brain, BookOpen, Clock, Target, CheckCircle2, Shield } from "lucide-react";
...
export default function StudentInsightPage() {
  ...
  const { data, isLoading } = trpc.teacher.getStudentEarlyWarning.useQuery({ studentId });
  ...
  if (!data) {
    return (
      <div className="glass mx-auto max-w-lg rounded-2xl p-8 text-center">
        <p className="text-sm text-neutral-500">No data available for this student</p>
  ...
  return (
    <div className="animate-fade-in space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-neutral-900 sm:text-2xl dark:text-white">{warning.studentName}</h1>
          <p className="text-sm text-neutral-500">Early Warning Analysis</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <div className="glass rounded-xl p-3 text-center">
          <div className={cn("text-2xl font-bold", summary.avgScore >= 60 ? "text-success" : "text-error")}>
            {summary.avgScore}%
          </div>
        ...
        <div className="glass rounded-xl p-3 text-center">
          <div className={cn("text-2xl font-bold", summary.avgRetention >= 60 ? "text-info" : "text-error")}>
            {summary.avgRetention}%
          </div>
          <div className="text-xs text-neutral-500">Avg Retention</div>
        </div>
      </div>

      {/* Signal Breakdown */}
      <div className="glass rounded-2xl p-4">
        <h3 className="mb-3 text-sm font-semibold text-neutral-700 dark:text-neutral-300">Risk Signal Breakdown</h3>
            ...
            <div key={signal.signal}>
              <div className="flex items-center justify-between text-xs">
                ...
                <span className={cn("font-bold", signal.score >= 70 ? "text-error" : signal.score >= 40 ? "text-warning" : "text-success")}>
                  {Math.round(signal.weightedScore)}/{Math.round(signal.weight * 100)}
                </span>
              </div>
              <div className="mt-1 h-2 overflow-hidden rounded-full bg-neutral-100 dark:bg-neutral-800">
                <div
                  className={cn("h-full rounded-full transition-all", signal.score >= 70 ? "bg-error" : signal.score >= 40 ? "bg-warning" : "bg-success")}
      ...
      {scoreTimeline.length > 0 && (
        <div className="glass rounded-2xl p-4">
          <h3 className="mb-3 text-sm font-semibold text-neutral-700 dark:text-neutral-300">Score Timeline</h3>
      ...
      {thetaTimeline.length > 0 && (
        <div className="glass rounded-2xl p-4">
          <h3 className="mb-3 text-sm font-semibold text-neutral-700 dark:text-neutral-300">Ability Estimate (θ) Over Time</h3>
          <div className="flex items-end gap-1">
            {thetaTimeline.map((point, i) => {
              const normalized = Math.max(0, Math.min(100, ((point.theta + 3) / 6) * 100));
              return (
                <div key={i} className="flex flex-1 flex-col items-center gap-1">
                  <span className="text-[9px] text-neutral-400">{point.theta.toFixed(2)}</span>
                  <div
                    className="w-full rounded-t bg-info"
                    style={{ height: `${Math.max(4, normalized * 0.8)}px` }}
                  />
                  <span className="text-[8px] text-neutral-400">{point.date?.slice(5) || ""}</span>
                </div>
              );
      ...
      {topicProgress.length > 0 && (
        <div className="glass rounded-2xl p-4">
          <h3 className="mb-3 text-sm font-semibold text-neutral-700 dark:text-neutral-300">Topic Progress</h3>
            ...
            {topicProgress.map((tp) => (
              <div key={tp.topicName} className="flex items-center gap-3">
                ...
                <span className="flex w-16 items-center justify-end gap-1 text-[10px]">
                  ...
                  <span className={tp.trend === "improving" ? "text-success" : tp.trend === "declining" ? "text-error" : "text-neutral-400"}>
                    {tp.trend}
                  </span>
                </span>
              </div>
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
            {retentionOverview.map((r) => (
              <div key={r.topicName} className={cn("rounded-xl border p-3", riskBg[r.status === "good" ? "none" : r.status])}>
                ...
                <p className="mt-1 text-[10px] text-neutral-500 capitalize">{r.status} retention</p>
              </div>
            ))}
          </div>
        </div>
