...
import {
  Search, Plus, Trash2, Eye, Edit2, HeartHandshake, X, CheckCircle2,
  AlertTriangle, ChevronLeft, ChevronRight, Upload, Download, FileText,
} from "lucide-react";
import { cn } from "@/lib/utils";

const DOMAIN_COLORS: Record<string, string> = {
  "Communication & Emotional Connection": "bg-pink-100 text-pink-700 dark:bg-pink-950 dark:text-pink-300",
  "Discipline & Behavior Management": "bg-rose-100 text-rose-700 dark:bg-rose-950 dark:text-rose-300",
  "Academic Support & Involvement": "bg-blue-100 text-blue-700 dark:bg-blue-950 dark:text-blue-300",
  "Health, Nutrition & Wellbeing": "bg-green-100 text-green-700 dark:bg-green-950 dark:text-green-300",
  "Safety & Supervision": "bg-amber-100 text-amber-700 dark:bg-amber-950 dark:text-amber-300",
  "Social Development": "bg-indigo-100 text-indigo-700 dark:bg-indigo-950 dark:text-indigo-300",
  "Digital Literacy & Screen Time": "bg-violet-100 text-violet-700 dark:bg-violet-950 dark:text-violet-300",
  "Cultural & Moral Values": "bg-purple-100 text-purple-700 dark:bg-purple-950 dark:text-purple-300",
  "Adolescent Development": "bg-fuchsia-100 text-fuchsia-700 dark:bg-fuchsia-950 dark:text-fuchsia-300",
  "Stress Management & Self-Care": "bg-teal-100 text-teal-700 dark:bg-teal-950 dark:text-teal-300",
  "Special Needs Awareness": "bg-cyan-100 text-cyan-700 dark:bg-cyan-950 dark:text-cyan-300",
  "Financial Literacy for Children": "bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300",
};
...
export default function AdminParentAssessmentsPage() {
  ...
  const { data: domains } = trpc.admin.getParentQuestionDomains.useQuery();
  const { data: qData, isLoading: questionsLoading } = trpc.admin.getParentQuestions.useQuery(
    { search, domain, page, pageSize },
  );

  const createQuestion = trpc.admin.createParentQuestion.useMutation({
    onSuccess: () => {
      setShowAddModal(false);
      setForm(emptyForm);
      setFormError("");
      utils.admin.getParentQuestions.invalidate();
      utils.admin.getParentQuestionDomains.invalidate();
    },
    onError: (err) => setFormError(err.message),
  });

  const updateQuestion = trpc.admin.updateParentQuestion.useMutation({
    onSuccess: () => {
  ...
  const deleteQuestion = trpc.admin.deleteParentQuestion.useMutation({
    onSuccess: () => {
  ...
  const bulkImport = trpc.admin.bulkImportParentQuestions.useMutation({
    onSuccess: (result) => {
  ...
  const handleExport = useCallback(async () => {
    const result = await utils.client.admin.exportParentQuestions.query({
    ...
    const csv = [
      "domain,dimension,questionText,option1,score1,option2,score2,option3,score3,option4,score4,option5,score5",
      ...result.map((q) => {
        ...
        return `"${q.domain}","${q.dimension || ""}","${q.questionText.replace(/"/g, '""')}",${optCols.map((c) => `"${c}"`).join(",")}`;
      }),
    ].join("\n");
    ...
    a.href = url;
    a.download = `parent-assessment-questions-${new Date().toISOString().split("T")[0]}.csv`;
    a.click();
  ...
  return (
    <div className="animate-fade-in space-y-6">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-xl font-bold text-neutral-900 sm:text-2xl dark:text-white">Parent Assessment Questions</h1>
          <p className="mt-1 text-sm text-neutral-500">Manage Likert-based parent self-assessment questions across all domains</p>
        </div>
      ...
      <div className="glass rounded-2xl p-4 sm:p-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base font-bold text-neutral-900 dark:text-white">Parent Self-Assessment</h2>
      ...
      {(showAddModal || editQuestion) && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-lg rounded-2xl bg-white p-5 sm:p-6 dark:bg-neutral-900 animate-slide-up max-h-[85vh] overflow-y-auto">
            <div className="flex items-start justify-between">
              <h3 className="text-lg font-bold text-neutral-900 dark:text-white">{editQuestion ? "Edit Parent Question" : "Add Parent Question"}</h3>
            ...
            <div className="mt-4 space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input className="w-full rounded-xl border border-neutral-200 bg-white px-3 py-3 text-sm outline-none focus:border-primary-500 dark:border-neutral-700 dark:bg-neutral-950 dark:text-white min-h-[44px]" placeholder="e.g. PAR-001" value={form.code} onChange={(e) => setForm((p) => ({ ...p, code: e.target.value }))} />
                </div>
                <div>
                  ...
                  <select className="w-full rounded-xl border border-neutral-200 bg-white px-3 py-3 text-sm outline-none dark:border-neutral-700 dark:bg-neutral-950 dark:text-white min-h-[44px]" value={form.domain} onChange={(e) => setForm((p) => ({ ...p, domain: e.target.value }))}>
                    {domains?.map((d) => <option key={d} value={d}>{d}</option>)}
                    {!domains && <option value="Communication & Emotional Connection">Communication & Emotional Connection</option>}
                  </select>
                </div>
              </div>
              <div>
                <label className="mb-1 block text-xs font-medium text-neutral-500">Dimension *</label>
                <input className="w-full rounded-xl border border-neutral-200 bg-white px-3 py-3 text-sm outline-none focus:border-primary-500 dark:border-neutral-700 dark:bg-neutral-950 dark:text-white min-h-[44px]" placeholder="e.g. Active Listening" value={form.dimension} onChange={(e) => setForm((p) => ({ ...p, dimension: e.target.value }))} />
              </div>
              <div>
